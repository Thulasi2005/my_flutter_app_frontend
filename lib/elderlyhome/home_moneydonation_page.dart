...
class PaymentRequest {
  ...
  Future<void> updateStatus(String newStatus) async {
    final response = await http.post(
      Uri.parse('http://10.0.2.2/donation_App/update_payment_request_status.php'),
      body: json.encode({
        'id': id,
...
class _DonationAndPaymentPageState extends State<DonationAndPaymentPage> with SingleTickerProviderStateMixin {
  ...
  Future<List<DonationRequest>> fetchDonationRequests() async {
    try {
      final response = await http.get(Uri.parse('http://10.0.2.2/donation_App/get_money_donation_requests.php'));
      if (response.statusCode == 200) {
  ...
  Future<List<PaymentRequest>> fetchPaymentRequests() async {
    try {
      final response = await http.get(Uri.parse('http://10.0.2.2/donation_App/get_payment_requests.php'));
      if (response.statusCode == 200) {
        List jsonResponse = json.decode(response.body);
